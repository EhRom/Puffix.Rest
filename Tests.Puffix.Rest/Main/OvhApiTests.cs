...
public class OvhApiTests
{
    ...
    public async Task GetPrfileTest()
    {
        ...
        try
        {
            ...
            IOvhApiQueryInformation authQueryInformation = httpRepository.BuildUnauthenticatedQuery(HttpMethod.Get, baseUri, authenticationUriPath, IOvhApiQueryInformation.EmptyQueryParameters, queryContent);
            string referenceUnixTime = await httpRepository.HttpAsync(authQueryInformation);
            token.SetReferenceUnixTime(referenceUnixTime);

            IOvhApiQueryInformation queryInformation = httpRepository.BuildAuthenticatedQuery(token, HttpMethod.Get, baseUri, accountUriPath, IOvhApiQueryInformation.EmptyQueryParameters, queryContent);
            string result = await httpRepository.HttpAsync(queryInformation);

            IOvhApiQueryInformation queryInformationBis = httpRepository.BuildAuthenticatedQuery(token, HttpMethod.Get, baseUri, logsUriPath, IOvhApiQueryInformation.EmptyQueryParameters, queryContent);
            ...
            IOvhApiQueryInformation queryInformationTer = httpRepository.BuildAuthenticatedQuery(token, HttpMethod.Get, baseUri, logUriPath, IOvhApiQueryInformation.EmptyQueryParameters, queryContent);
    ...
    public async Task UnitTest()
    {
        ...
        try
        {
            ...
            // Test
            IQueryContent queryContent = EmptyContent.CreateNew();
            IOvhApiQueryInformation queryInformation = httpRepository.BuildAuthenticatedQuery(token, HttpMethod.Get, baseUri, logsUriPath, IOvhApiQueryInformation.EmptyQueryParameters, queryContent);
