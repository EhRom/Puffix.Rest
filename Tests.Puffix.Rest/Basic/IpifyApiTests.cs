...
public class IpifyApiTests
{
    ...
    [Test]
    public async Task UnitTestWithStatus()
    {
        try
        {
            BuildMocks(container, out IIpifyApiToken token, out Mock<IHttpClientFactory> httpClientFactoryMock, out IIpifyApiBasicHttpRepository httpRepository);
            ...
            {
                Assert.That(actualResult, Is.Not.Null);
                Assert.That(actualResult.ErrorContent, Is.Not.Null.And.Empty);
                Assert.That(actualResult.IsSuccess, Is.True);
                Assert.That(actualResult.ResultCode, Is.EqualTo(HttpStatusCode.OK));
                Assert.That(actualResult.ResultContent, Is.Not.Null.And.Not.Empty);
                Assert.That(actualResult.ResultContent, Is.EqualTo(sampleResponse));
                Assert.That(actualResult.Headers, Is.Not.Null);
                Assert.That(actualResult.Headers.Count, Is.EqualTo(0));
            });
        }
        catch (Exception error)
        {
